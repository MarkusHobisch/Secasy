...
/* Portable format handling for long double (MSVC does not support %Lf) */
#if defined(_MSC_VER)
  #define LD_FMT "%0.6f"
...
static unsigned int g_messages = 50;          // -m number of random base messages
static size_t g_inputLen = 64;                // -l bytes per message
static unsigned int g_sampledBitFlips = 64;   // -B number of bit flips sampled per base message (0 = all bits)
static unsigned long long g_seed = 0;         // -s seed (0 => time based)
static int g_flagHistogram = 0;               // -H print bucket histogram
static int g_flagQuiet = 0;                   // -q suppress per-assessment text (just stats)
static int g_flagExtended = 0;                // -X extended analysis (bit bias, entropy, multi-bit)

// Extended mode data structures
static unsigned long long *g_bitChanged = NULL;
static unsigned long long *g_bitCompared = NULL;
static size_t g_bitCapacity = 0; // number of bits allocated

static unsigned long long g_byteFreq[256] = {0};
static unsigned long long g_totalHistogramBytes = 0ULL;

// Multi-bit flip test (k = 2,4,8)
static const int g_multiKCount = 3;
static const int g_multiKVals[3] = {2,4,8};
static unsigned long long g_multiTotalFlips[3] = {0}; // number of multi-bit trials per k
...
// Replaced long double accumulators with double for better Windows portability
static double g_sumRatios = 0.0;      // sum of (hd / bitsCompared) per flip
static double g_sumSqRatios = 0.0;    // sum of squares of ratios

// Histogram buckets of per-flip avalanche ratio (0-0.1, 0.1-0.2, ... 0.9-1.0)
static unsigned long long g_histBuckets[10] = {0};

// Simple xorshift64* PRNG (deterministic & fast; not cryptographically secure)
...
static void parse_args(int argc, char** argv) {
    int opt;
    while ((opt = getopt(argc, argv, "m:l:B:r:n:i:s:HqXh")) != -1) {
        switch (opt) {
...
static void random_buffer(unsigned char* buf, size_t len) {
...
static int popcount8(unsigned int x) {
    ...
    // Simple table-based or bit-fiddling implementation
    x &= 0xFFu;
    x = x - ((x >> 1) & 0x55u);
    x = (x & 0x33u) + ((x >> 2) & 0x33u);
    return (int)((((x + (x >> 4)) & 0x0Fu) * 0x01u) & 0x3Fu);
#else
    // Generic fallback
...
// Convert hex (without 0x prefix) to bytes; returns number of bits produced
static size_t hex_to_bits(const char* hex, unsigned char** outBytes) {
    size_t len = strlen(hex);
    size_t outLen = (len + 1) / 2; // bytes
    unsigned char* bytes = (unsigned char*)malloc(outLen ? outLen : 1);
    if (!bytes) return 0;
    size_t i = 0, j = 0;
    if (len & 1) {
        char c = hex[0];
        ...
        else val = 0; // non-hex gracefully -> 0
        bytes[j++] = (unsigned char)val; // high nibble only (lower 4 bits meaningful)
        i = 1;
    }
    for (; i < len; i += 2) {
        char c1 = hex[i];
        char c2 = hex[i+1];
        int v1, v2;
        if (c1 >= '0' && c1 <= '9') v1 = c1 - '0'; else if (c1 >= 'a' && c1 <= 'f') v1 = c1 - 'a' + 10; else if (c1 >= 'A' && c1 <= 'F') v1 = c1 - 'A' + 10; else v1 = 0;
        if (c2 >= '0' && c2 <= '9') v2 = c2 - '0'; else if (c2 >= 'a' && c2 <= 'f') v2 = c2 - 'a' + 10; else if (c2 >= 'A' && c2 <= 'F') v2 = c2 - 'A' + 10; else v2 = 0;
        bytes[j++] = (unsigned char)((v1 << 4) | v2);
    }
    *outBytes = bytes;
    return outLen * 8; // bits
}

// If hex lengths differ (because leading zeros suppressed), left-pad shorter with zeros
static void normalize_hex(const char* a, const char* b, char** outA, char** outB, size_t* outLen) {
    size_t la = strlen(a); size_t lb = strlen(b);
    size_t L = la>lb?la:lb;
    char* na = (char*)malloc(L+1); char* nb = (char*)malloc(L+1);
    if(!na||!nb){fprintf(stderr,"OOM\n"); exit(EXIT_FAILURE);}
    memset(na,'0',L); memset(nb,'0',L);
    memcpy(na+(L-la), a, la); memcpy(nb+(L-lb), b, lb);
    na[L]='\0'; nb[L]='\0';
    *outA=na; *outB=nb; *outLen=L;
}

static void single_hash(const unsigned char* data, size_t len, char** outHex) {
    initFieldWithDefaultNumbers(maximumPrimeIndex);
    processBuffer(data, len);
    char* h = calculateHashValue();
    *outHex = h; // caller frees
}

static void record_sample(int hd, int bitsCompared) {
    if (bitsCompared <= 0) return;
...
static void extended_record_bitwise(const unsigned char* a, const unsigned char* b, size_t bitsUsed) {
    if (!g_flagExtended || bitsUsed == 0) return;
    ensure_bit_capacity(bitsUsed);
    size_t fullBytes = bitsUsed / 8;
    size_t remBits = bitsUsed % 8;
    size_t bitIndex = 0;
    for (size_t i=0; i<fullBytes; ++i) {
        unsigned char delta = (unsigned char)(a[i]^b[i]);
        for (int bBit=0; bBit<8; ++bBit) {
            g_bitCompared[bitIndex]++;
            if (delta & (1u<<bBit)) g_bitChanged[bitIndex]++;
            bitIndex++;
        }
    }
    if (remBits) {
        unsigned char delta = (unsigned char)(a[fullBytes]^b[fullBytes]);
        for (size_t bBit=0; bBit<remBits; ++bBit) {
            g_bitCompared[bitIndex]++;
            if (delta & (1u<<bBit)) g_bitChanged[bitIndex]++;
            bitIndex++;
        }
...
static void extended_record_bytes(const unsigned char* a, const unsigned char* b, size_t bytes) {
    if (!g_flagExtended) return;
    for (size_t i=0;i<bytes;i++) {
        g_byteFreq[a[i]]++; g_byteFreq[b[i]]++; // count both original & modified hash bytes
...
static void perform_multi_bit_trials(const unsigned char* base, size_t lenBytes, const char* baseHex) {
    ...
    // For each k run limited number of trials (cap to e.g. 32 or totalBits/2 whichever smaller)
    ...
    for (int ki=0; ki<g_multiKCount; ++ki) {
        ...
        for (int t=0; t<trials; ++t) {
            ...
            char* modHex = NULL; single_hash(temp, lenBytes, &modHex);
            if (!modHex) { free(temp); continue; }
            // Normalize & compare
            char *normA=NULL,*normB=NULL; size_t hexLen=0; normalize_hex(baseHex, modHex, &normA, &normB, &hexLen);
            unsigned char *bytesA=NULL,*bytesB=NULL; size_t bitsA = hex_to_bits(normA, &bytesA); size_t bitsB = hex_to_bits(normB, &bytesB);
            int hd; int usedBits;
            if (bitsA != bitsB) {
                ...
                size_t bytes = (bitsA+7)/8; hd = hamming_bits(bytesA, bytesB, bytes); usedBits = (int)bitsA; }
            g_multiHammingBits[ki] += (unsigned long long)hd;
            g_multiBitsCompared[ki] += (unsigned long long)usedBits;
            g_multiTotalFlips[ki]++;
            // Extended recording (bit / byte level) optional: skip to save time
...
int main(int argc, char** argv) {
    ...
    single_hash(base, g_inputLen, &baseHex);
        if(!baseHex){fprintf(stderr,"hash failed base\n"); free(base); return 1;}
        ...
        for (unsigned int f=0; f<flipsThisMsg; ++f) {
            ...
            if (g_flagExtended) {
                size_t bytesUsed = (size_t)usedBits / 8; if ((size_t)usedBits % 8) bytesUsed++;
                extended_record_bitwise(bytesA, bytesB, (size_t)usedBits);
                extended_record_bytes(bytesA, bytesB, bytesUsed);
            }

            free(bytesA); free(bytesB);
            free(normA); free(normB);
            free(modHex);
            base[byteIndex] = original;
        }

        if (g_flagExtended) {
            // Perform multi-bit trials referencing this base message
            perform_multi_bit_trials(base, g_inputLen, baseHex);
        }
        free(baseHex);
    }

    double elapsed = wall_time_seconds() - start;
    // AFTER the main loop (replace long double stats section)
    double meanAvalanche = (g_totalBitsCompared>0) ? ((double)g_totalHammingBits / (double)g_totalBitsCompared) : 0.0;
    double meanRatio = (g_totalFlipsPerformed>0) ? (g_sumRatios / (double)g_totalFlipsPerformed) : 0.0;
    double variance = 0.0;
    if (g_totalFlipsPerformed > 1) {
        double m = meanRatio;
        variance = (g_sumSqRatios / (double)g_totalFlipsPerformed) - (m * m);
        if (variance < 0) variance = 0;
    }
    double stddev = (variance > 0) ? sqrt(variance) : 0.0;
    double p = meanAvalanche;
    double stderrBits = (g_totalBitsCompared>0) ? sqrt(p*(1.0-p) / (double)g_totalBitsCompared) : 0.0;
    double ci95_low = p - 1.96 * stderrBits;
    double ci95_high = p + 1.96 * stderrBits;
    if (ci95_low < 0) ci95_low = 0; if (ci95_high > 1) ci95_high = 1;
    double zScore = (stderrBits>0) ? (p - 0.5) / stderrBits : 0.0;

    /* Portable printing using simple double formats */
    ...
    printf("Mean avalanche rate (bit-level): %.6f\n", meanAvalanche);
    printf("Mean per-flip ratio: %.6f\n", meanRatio);
    printf("Stddev per-flip ratio: %.6f\n", stddev);
    printf("95%% CI (bit-level p): [%.6f , %.6f]\n", ci95_low, ci95_high);
    printf("Z-score vs 0.5: %.6f\n", zScore);
    // Extra raw debug line to verify arithmetic
    printf("(DEBUG) raw_fraction = %llu / %llu = %.6f\n", (unsigned long long)g_totalHammingBits, (unsigned long long)g_totalBitsCompared, meanAvalanche);
    printf("Time: %.3f s (%.2f flips/s)\n", elapsed, elapsed>0? (double)g_totalFlipsPerformed/elapsed:0.0);

    if (!g_flagQuiet) {
        if (meanAvalanche < 0.40) {
            printf("Assessment: Low diffusion under tested parameters (substantially below 0.5).\n");
        } else if (meanAvalanche < 0.47) {
            printf("Assessment: Moderate diffusion (below ideal).\n");
        } else if (meanAvalanche < 0.53) {
            printf("Assessment: Near target diffusion.\n");
        } else {
            printf("Assessment: >0.53 (could be acceptable or indicate structural artifacts).\n");
        }
    }

    if (g_flagHistogram) {
        printf("Histogram (ratio buckets 0.0-0.1 ... 0.9-1.0):\n");
        unsigned long long total = g_totalFlipsPerformed ? g_totalFlipsPerformed : 1ULL;
        for (int i=0;i<10;i++) {
            double pct = (double)g_histBuckets[i] * 100.0 / (double)total;
            printf("  [%d] %.2f%% (%llu)\n", i, pct, (unsigned long long)g_histBuckets[i]);
        }
    }

    if (g_flagExtended) {
        // Per-bit bias summary
        if (g_bitCapacity > 0) {
            double sumP = 0.0, sumSqP = 0.0; double minP = 1.0, maxP = 0.0; unsigned long long countedBits=0; unsigned long long outOfBand=0;
            for (size_t i=0;i<g_bitCapacity;i++) {
                unsigned long long comp = g_bitCompared[i];
                if (comp==0) continue;
                double pv = (double)g_bitChanged[i] / (double)comp;
                sumP += pv; sumSqP += pv*pv; if (pv<minP) minP=pv; if (pv>maxP) maxP=pv; countedBits++; if (pv<0.45 || pv>0.55) outOfBand++;
            }
            if (countedBits>0) {
                double meanP = sumP / (double)countedBits;
                double varP = (sumSqP/(double)countedBits) - meanP*meanP; if (varP<0) varP=0;
                double sdP = sqrt(varP);
                printf("--- Extended: Per-bit bias ---\n");
                printf("Bits observed: %llu\n", (unsigned long long)countedBits);
                printf("Min bit flip rate: %.4f Max: %.4f Mean: %.4f SD: %.4f Out-of-[0.45,0.55]: %llu\n", minP, maxP, meanP, sdP, (unsigned long long)outOfBand);
            }
        }
        // Byte entropy
        if (g_totalHistogramBytes > 0) {
            long double H = 0.0L;
            for (int i=0;i<256;i++) {
                unsigned long long c = g_byteFreq[i];
                if (!c) continue;
                long double pbyte = (long double)c / (long double)g_totalHistogramBytes;
                H -= pbyte * (log(pbyte)/log(2.0L));
            }
            long double maxH = log(256.0L)/log(2.0L); // 8
            printf("--- Extended: Output byte distribution ---\n");
            printf("Bytes sampled: %llu Entropy: %.4Lf / 8.0000 (%.2Lf%% of max)\n", (unsigned long long)g_totalHistogramBytes, H, (H/maxH)*100.0L);
        }
        // Multi-bit trials
        int anyMulti = 0; for (int i=0;i<g_multiKCount;i++) if (g_multiTotalFlips[i]>0) { anyMulti=1; break; }
        if (anyMulti) {
            printf("--- Extended: Multi-bit flip diffusion ---\n");
            for (int i=0;i<g_multiKCount;i++) {
                if (g_multiTotalFlips[i]==0) continue;
                double ratio = (g_multiBitsCompared[i]>0)? (double)g_multiHammingBits[i]/(double)g_multiBitsCompared[i] : 0.0;
                printf("k=%d trials=%llu mean_ratio=%.6f\n", g_multiKVals[i], (unsigned long long)g_multiTotalFlips[i], ratio);
            }
        }
    }
    ...
    free(base);
    free(g_bitChanged); free(g_bitCompared);
    return 0;
}
