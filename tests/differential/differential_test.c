...
void test_sparse_inputs(int maxPrimeIndex)
{
    ...
    int passed = 1;
    if (dist_zeros_ones < expected * 0.4)
    {
        printf("  ⚠ Zeros-Ones distance too low!\n");
        passed = 0;
    }
    if (dist_zeros_single < expected * 0.4)
    {
        printf("  ⚠ Zeros-Single distance too low!\n");
        passed = 0;
    }
    if (dist_zeros_alt < expected * 0.4)
    {
        printf("  ⚠ Zeros-Alt distance too low!\n");
        passed = 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
        maxPrimeIndex = atoi(argv[1]);
    if (argc > 2)
        numberOfRounds = atol(argv[2]);
    if (argc > 3)
        hashLengthInBits = atoi(argv[3]);
