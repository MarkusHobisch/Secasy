 ...
 *
 * METHOD:
 *   - Global mean avalanche rate across random messages
 *   - (Extended -X) Per-bit flip frequencies  → bias band detection
 *   - (Extended -X) Multi-bit flip diffusion  (k = 2,4,8) → saturation check
 *   - (Extended -X) Experimental entropy sampler (advisory only)
 *   Use sampled flips (-B > 0) for speed or exhaustive (-B 0) for precision.
 *   Increase rounds to test diffusion depth scaling.
 *
 * LIMITATIONS:
 *   Does not cover SAC matrix, higher-order correlations, or adversarial
 *   inputs. Passing does not prove cryptographic strength — it is an
 *   early health indicator only.
 *
 * CONCLUSION:
 *   Mean avalanche rate is ≈ 50.0 % ± 0.3 % at 10 rounds for all hash
 *   sizes. Per-bit bias stays well within ± 2 %. Multi-bit diffusion
 *   saturates by k = 4.
 *
 * BUILD TARGET: SecasyAvalanche
...
{
    if (!g_flagSAC || !g_sacMatrix || !g_sacCompared)
        return;
    if (inputBit >= g_sacInputBits)
        return;
    ...
    for (size_t i = 0; i < fullBytes; i++)
    {
        ...
        {
            if (outBitIndex >= g_sacOutputBits)
                break;
...
{
    int opt;
    while ((opt = getopt(argc, argv, "m:l:B:r:n:i:s:S:HqXh")) != -1)
    {
        switch (opt)
        {
        case 'm':
    ...
    if (g_messages == 0 || g_inputLen == 0 || numberOfRounds == 0)
    {
        fprintf(stderr, "Invalid zero parameter\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (g_flagExtended)
    {
        ...
        if (g_totalHistogramBytes > 0)
        {
            ...
            for (int i = 0; i < 256; i++)
            {
                ...
                H -= pbyte * (logl(pbyte) / logl(2.0L));
            }
            long double maxH = logl(256.0L) / logl(2.0L);
