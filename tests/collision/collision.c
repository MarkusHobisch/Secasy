...
{
    if (s == 0)
        s = (uint64_t)time(NULL) * 0x9e3779b97f4a7c15ULL;
    rng_state = s;
}
...
static uint64_t hash_hex(const char *s)
{
...
static size_t tableCap = 0;
static size_t tableCount = 0;

static int table_init(size_t cap)
{
    tableCap = 1;
    while (tableCap < cap * 2)
        tableCap <<= 1;
    table = (Entry *)calloc(tableCap, sizeof(Entry));
    return table ? 0 : -1;
}
...
static int table_insert_or_collision(const char *hex)
{
    ...
    while (1)
    {
        if (table[idx].hex == NULL)
        {
            ...
            if (!table[idx].hex)
            {
                fprintf(stderr, "OOM storing hash string\n");
                exit(EXIT_FAILURE);
            }
            tableCount++;
            return 0; /* new */
...
int main(int argc, char **argv)
{
    ...
    while (optIndex < argc)
    {
        if (strcmp(argv[optIndex], "-m") == 0 && optIndex + 1 < argc)
        {
            messages = (size_t)strtoull(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-l") == 0 && optIndex + 1 < argc)
        {
            lenBytes = (size_t)strtoull(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-r") == 0 && optIndex + 1 < argc)
        {
            numberOfRounds = strtoul(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-n") == 0 && optIndex + 1 < argc)
        {
            hashLengthInBits = (int)strtoul(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-s") == 0 && optIndex + 1 < argc)
        {
            seed = strtoull(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-T") == 0 && optIndex + 1 < argc)
        {
            truncBits = (int)strtol(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-X") == 0 && optIndex + 1 < argc)
        {
            ...
            while (tok && sweepCount < (int)(sizeof(sweepBits) / sizeof(sweepBits[0])))
            {
                ...
                if (b > 0 && b <= 256)
                {
                    sweepBits[sweepCount++] = b;
                }
                tok = strtok(NULL, ",");
            }
            free(dup);
        }
        else if (strcmp(argv[optIndex], "-F") == 0)
        {
            doFreq = 1;
        }
        else if (strcmp(argv[optIndex], "-P") == 0)
        {
            doPos = 1;
        }
        else if (strcmp(argv[optIndex], "-p") == 0 && optIndex + 1 < argc)
        {
            detailPos = strtol(argv[++optIndex], NULL, 10);
        }
        else if (strcmp(argv[optIndex], "-B") == 0 && optIndex + 1 < argc)
        {
            byteAnalyze = strtol(argv[++optIndex], NULL, 10);
        }
        else
        {
            usage(argv[0]);
    ...
    for (size_t m = 0; m < messages; ++m)
    {
        ...
        if (doFreq || doPos || detailPos >= 0 || byteAnalyze > 0)
        {
            ...
            if ((doPos || detailPos >= 0) && observedMaxHexLen < len)
            {
                ...
                if (!newMatrix)
                {
                    fprintf(stderr, "OOM posFreq matrix\n");
                    exit(EXIT_FAILURE);
                }
                if (posFreq)
                {
        ...
        else
        {
            if (truncBits > 0)
            {
                ...
                {
                    if (table_insert_or_collision(hv))
                        collisions++;
                }
            }
            else
    ...
    if (!sweepMode)
    {
        ...
        if (truncBits > 0)
            printf("(Truncation active: %d bits)\n", truncBits);
...
#define M_PI 3.14159265358979323846
    ...
    else
    {
        ...
        for (int si = 0; si < sweepCount; ++si)
        {
            ...
            if (table_init(messages) != 0)
            {
                fprintf(stderr, "Failed to init table (sweep)\n");
                return 1;
            }
            size_t localColl = 0;
            for (size_t m = 0; m < messages; ++m)
            ...
            double expectedColl = 0.0;
            double birthdayApprox = 0.0;
            int approxUsed = 0;
            if (kbits <= 60.0)
            {
                double space = pow(2.0, kbits);
        ...
        if (totalB > 0)
        {
            ...
            for (int i = 0; i < 256; i++)
            {
                ...
                for (int k = 0; k < show; k++)
                {
                    if (diff > bestDev[k])
                    {
                        for (int s = show - 1; s > k; s--)
                        {
                            bestDev[s] = bestDev[s - 1];
