...
{
    if (a->px != b->px || a->py != b->py) return 0;
    for (int i = 0; i < FIELD_SIZE; i++)
        for (int j = 0; j < FIELD_SIZE; j++)
        ...
        for (int bi = 0; bi < MSG_LEN; bi++) {
            for (int bit = 0; bit < 8; bit++) {
                ...
                if (hamming_hex(h0c, h1) == 0) {
                    ...
                    int diff_sub = -1;
                    for (int s = 0; s < 4; s++)
                        if (orig_dirs[s] != flip_dirs[s]) { diff_sub = s; break; }
                    ...
                    if (grids_identical(&snap_orig, &snap_flip)) {
                        ...
                        for (int s = 0; s < diff_sub; s++) {
                            ...
                            int d = (msg[bi] >> (s*2)) & 3;
                            switch (d) {
                            case 0: pos.y=(uint32_t)((pos.y-op+1u)&mask); pos.x=(pos.x+(pos.y>>1)+1u)&mask; break;
                            case 1: pos.x=(uint32_t)((pos.x+op+1u)&mask); pos.y=(pos.y+(pos.x>>1)+4u)&mask; break;
                            case 2: pos.x=(uint32_t)((pos.x-op)&mask); pos.y=(pos.y+(pos.x>>1)+2u)&mask; break;
                            case 3: pos.y=(uint32_t)((pos.y+op)&mask); pos.x=(pos.x+(pos.y>>1)+3u)&mask; break;
                            }
                        }
                        ...
                        if (is_dir_coll) phase2_dir_coll++;
        ...
        for (int bi = 0; bi < MSG_LEN; bi++) {
            for (int bit = 0; bit < 8; bit++) {
                ...
                if (hamming_hex(h0c, h1) == 0 &&
                    ...
                    total_coll++;
                    by_byte[bi]++;
                    int sub = (bit < 2) ? 0 : (bit < 4) ? 1 : (bit < 6) ? 2 : 3;
                    by_sub[sub]++;
    ...
    *path_len = 0;
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        for (int s = 0; s < 4; s++) {
            ...
            uint32_t mask = FIELD_SIZE - 1;
            switch (dirs[s]) {
            case 0: pos.y=(uint32_t)((pos.y-oldPrime+1u)&mask); pos.x=(pos.x+(pos.y>>1)+1u)&mask; break;
            case 1: pos.x=(uint32_t)((pos.x+oldPrime+1u)&mask); pos.y=(pos.y+(pos.x>>1)+4u)&mask; break;
            case 2: pos.x=(uint32_t)((pos.x-oldPrime)&mask); pos.y=(pos.y+(pos.x>>1)+2u)&mask; break;
            case 3: pos.y=(uint32_t)((pos.y+oldPrime)&mask); pos.x=(pos.x+(pos.y>>1)+3u)&mask; break;
            }
        }
    }
            ...
            for (int bit = 0; bit < 8; bit++) {
                ...
                if (hamming_hex(h0c, h1) == 0 &&
                    ...
                    if (diff_count == 0) {
                        ...
                        if (!first_zero_shown) {
                            first_zero_shown = 1;
                            int diff_step = bi * 4 + ((bit < 2) ? 0 : (bit < 4) ? 1 : (bit < 6) ? 2 : 3);
                        ...
                        if (!first_nonzero_shown) {
                            ...
                            for (int s2 = 0; s2 < lenA; s2++) visitA[pathA[s2][0]][pathA[s2][1]]++;
                            for (int s2 = 0; s2 < lenB; s2++) visitB[pathB[s2][0]][pathB[s2][1]]++;
                            int multiset_ok = 1;
                            for (int ii = 0; ii < FIELD_SIZE; ii++)
                                for (int jj = 0; jj < FIELD_SIZE; jj++)
                                    if (visitA[ii][jj] != visitB[ii][jj]) multiset_ok = 0;
