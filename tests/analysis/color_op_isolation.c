                             ...
                             const char *verdict)
{
    ...
    for (int i = 0; i < N_BINS; i++) {
        ...
        if (i == ideal_bin)
        {
            ...
            for (int b = bar; b < BAR_MAX_W; b++) putchar('.');
            printf(" <==IDEAL |\n");
        }
        else
        {
                     ...
                     double *out_min, double *out_max)
{
    ...
    *out_max    = max_pct;
    ...
    const char *verdict;
    if      (mean >= 45.0 && mean <= 55.0 && stddev <= 8.0)  verdict = "STRONG  -- ideal avalanche, mass concentrated near 50%";
    else if (mean <  10.0)                                    verdict = "BROKEN  -- collapsed to 0%: output converges regardless of input";
    else if (mean >  90.0)                                    verdict = "BROKEN  -- collapsed to 100%: all bits always flip";
    else if (mean <  30.0 || mean > 70.0)                     verdict = "WEAK    -- heavily biased, far from ideal";
    else                                                      verdict = "PARTIAL -- some diffusion, but significantly degraded";
...
int main(void)
{
    ...
    typedef struct { const char *label; int force; ColorIndex_t op; } Mode;
    ...
    for (int i = 0; i < n_modes; i++) {
        const char *verdict;
        if      (means[i] >= 45.0 && means[i] <= 55.0 && stddevs[i] <= 8.0) verdict = "STRONG";
        else if (means[i] < 10.0 || means[i] > 90.0)                         verdict = "BROKEN";
        else if (means[i] < 30.0 || means[i] > 70.0)                         verdict = "WEAK";
